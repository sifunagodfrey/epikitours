...
$metaDescription = "Add a new booking to EpikiTours admin panel.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO epi_bookings (customer_id, tour_id, tour_guide, total_amount, status, booking_date)
                VALUES (:customer_id, :tour_id, :tour_guide, :total_amount, :status, :booking_date)
            ");
            $stmt->execute([
            ...
            <div class="mb-3">
                ...
                <select name="tour_guide" id="tour_guide" class="form-control" required>
                    ...
                    <?php foreach ($guides as $g): ?>
                        <option value="<?= $g['id'] ?>"><?= htmlspecialchars($g['first_name'].' '.$g['last_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
